...
public:
    ...
    */
    virtual bool Mark(const unsigned int flags)
    {
		...
		void *const markRecordMemory = Alloc(sizeof(MarkRecord), "MarkRecord", flags, EASTL_ALLOCATOR_MIN_ALIGNMENT);
        if (!markRecordMemory)
        {
		...
		EA_ASSERT(EA::Physics::IsMemAligned(markRecordMemory, EASTL_ALLOCATOR_MIN_ALIGNMENT));
        MarkRecord *const record = new (markRecordMemory) MarkRecord;

        EA_ASSERT(record);
		EA_ASSERT(EA::Physics::IsMemAligned(record, EASTL_ALLOCATOR_MIN_ALIGNMENT));
    ...
    */
    virtual bool Release(const unsigned int flags)
    {
        if (flags & EA::Allocator::MEM_PERM)
        {
            ...
            // The pointer needs to be aligned forward, as it will have been aligned backwards during allocation, unless the mark was the very first
            // allocation made, in which case the pointer should point at (or aligned slightly beyond) the end of the buffer.
            m_highPosition = EA::Physics::MemAlign(m_highPosition, EASTL_ALLOCATOR_MIN_ALIGNMENT);
            m_highPosition = m_highPosition > m_end ? m_end : m_highPosition;
            m_highMark = newMark;
        ...
        else
        {
            ...
            // The pointer needs to be aligned backwards, as it will have been aligned forwards during allocation, unless the mark was the very first
            // allocation made, in which case the pointer should point at  (or aligned slightly beyond) the start of the buffer.
            AlignPointerBackward(m_lowPosition, EASTL_ALLOCATOR_MIN_ALIGNMENT);
            m_lowPosition = m_lowPosition < m_start ? m_start : m_lowPosition;
            m_lowMark = newMark;
    ...
    */
    virtual uint32_t LargestAllocatableSize(const unsigned int /*flags*/, const uint32_t alignment) const
    {
        ...
        allocation = EA::Physics::MemAlign(allocation, alignment);
    ...
    */
    ...
    {
        // Forward to the more general method, with a default minimum alignment.
        return Alloc(size, name, flags, EASTL_ALLOCATOR_MIN_ALIGNMENT, 0);
    }
    ...
    */
    virtual void *Alloc(size_t size, const char * /*name*/, unsigned int flags, unsigned int align, unsigned int /*alignOffset*/ = 0)
    {
        ...
        {
            allocation = EA::Physics::MemAlign(m_lowPosition, align);
